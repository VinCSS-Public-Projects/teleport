...
param(
    ...
    # libfido2 (static) is enabled by default so tsh/tctl get MFA support.
    # Disable with -Fido2:$false.
    [switch]$Fido2 = $true,
    [switch]$Piv,
    [switch]$PurgeCaches
)
...
$Script:CacheVolumes = [ordered]@{
    ...
    "teleport-buildbox-rustup-$Arch"       = '/home/ci/.rustup'
...
function Get-DockerRunArgs {
    ...
    $args += @(
        ...
        # Shadow the image's root-owned RUSTUP_HOME (/usr/local/rustup) with a
        # ci-owned volume. The prebuilt buildbox bakes an older default toolchain
        # (e.g. 1.81.0), but rust-toolchain.toml pins a newer one; the ci user
        # can't install it into /usr/local/rustup, which trips the Makefile's
        # rustup-toolchain-warning and breaks the rdpclient/wasm cargo builds.
